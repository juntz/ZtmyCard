...
var ready_status = {}
@onready var chronos: ChronosNode = $"../Chronos"
...
func select_card(player: PlayerNode, card: CardNode):
	var card_field = player.find_card_field(card)
	if card_field == PlayerNode.Field.SELECTION:
		card.selectable = false
		card.unset_hover()
		_move_card(player, card, PlayerNode.Field.ABYSS)
		player.draw_require_count += 1
		return true
	
	if card_field == PlayerNode.Field.HAND:
		if player.controllable:
			pass
		if player.battle_field_card() == null && (card.info["type"] == "character"):
			_move_card(player, card, PlayerNode.Field.BATTLE)
			player.draw_require_count += 1
		elif player.get_cards(PlayerNode.Field.SET).size() < player.setable_card_count:
			_move_card(player, card, PlayerNode.Field.SET)
			player.draw_require_count += 1
		else:
			return false
		return true
	
	_move_card(player, card, PlayerNode.Field.HAND)
	player.draw_require_count -= 1
	return true
...
func next_phase_ready():
	...
	print("PlayerNode " + str(id) + " is ready for the next phase (" + Phase.keys()[next_phase] + ").")
...
func _move_card(player: PlayerNode, card: CardNode, to: PlayerNode.Field):
	var from = player.find_card_field(card)
	var idx = player.find_card_index(card, from)
	move_card.rpc(from, idx, to)


func _add_card(number: int, to: PlayerNode.Field):
...
func add_card(number : int, to: PlayerNode.Field):
...
func open_card(from, idx, card_number):
	var player = _get_player()
	var from_field = player.card_fields[from]
	var card: CardNode = from_field.cards()[idx]
...
func finish_mulligan():
	var player = _get_player()
	for card in player.get_cards(PlayerNode.Field.SELECTION):
		_move_card(player, card, PlayerNode.Field.HAND)
	for card in player.get_cards(PlayerNode.Field.ABYSS):
		card.close_card()
		card.selectable = false
		_move_card(player, card, PlayerNode.Field.DECK)
	player.card_fields[PlayerNode.Field.DECK].shuffle()


func _ready_mulligan():
	var player = _get_player()
	for i in range(MULLIGAN_COUNT):
		draw_card(player, PlayerNode.Field.SELECTION)
...
func draw_card(player: PlayerNode, to = PlayerNode.Field.HAND):
	var deck_cards = player.get_cards(PlayerNode.Field.DECK)
	if deck_cards.size() == 0:
		return
	
	var card: CardNode = deck_cards[-1]
...
func _draw_cards():
	for player: PlayerNode in players.values():
...
func _drop_card(player: PlayerNode, card: CardNode):
	var target_card_field = PlayerNode.Field.POWER_CHARGER \
		if int(card.info["sendToPower"]) > 0 else \
		PlayerNode.Field.ABYSS
	_move_card(player, card, target_card_field)


func _set_player_hand_card_selectable(selectable: bool):
	for card in _get_player().get_cards(PlayerNode.Field.HAND):
		card.selectable = selectable


func _open_cards():
	var player = _get_player()
	for field in [PlayerNode.Field.BATTLE, PlayerNode.Field.SET]:
...
func _on_enchant_end():
	var player = _get_player()
	var cards = player.get_cards(PlayerNode.Field.SET)
	if cards.size() > 0:
		var card = cards[0]
		_move_card(player, card, PlayerNode.Field.ENCHANT)
...
func _swap_battle_field_card(player: PlayerNode):
	var set_field_cards = player.get_cards(PlayerNode.Field.SET)
	for i in set_field_cards.size():
		var card = set_field_cards[i]
		if card.info["type"] != "character":
			continue
		
		var battle_field_cards = player.get_cards(PlayerNode.Field.BATTLE)
		if battle_field_cards.size() == 0:
			return
		
		var drop_target_field = PlayerNode.Field.ABYSS
		if battle_field_cards[0].info["sendToPower"] > 0:
			drop_target_field = PlayerNode.Field.POWER_CHARGER
		move_card.rpc(PlayerNode.Field.BATTLE, 0, drop_target_field)
		move_card.rpc(PlayerNode.Field.SET, i, PlayerNode.Field.BATTLE)
		return
...
func _end_battle():
	for player: PlayerNode in players.values():
		player.end_battle()
	var player = _get_player()
	for card in player.get_cards(PlayerNode.Field.ENCHANT):
